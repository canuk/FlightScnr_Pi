...
"""Load cached aircraft photo surfaces for the detail screen."""
...
def load_photo_surface(
    ...
    *,
    ...
    """Load an aircraft photo scaled to fit max_h (and optional max_w)."""
    ...
    try:
        ...
        if radius > 0 and ImageDraw is not None:
            ...
            ImageDraw.Draw(mask).rounded_rectangle(
                (0, 0, new_w - 1, new_h - 1),
                radius=min(radius, new_w // 4, new_h // 4),
                fill=255,
            )
            image.putalpha(mask)
