...
1. Swipe up → **About** → swipe left → **Settings** → **Display** → tap **Facing**
...
| **[LiveATC](https://www.liveatc.net/)**                                | Optional                 | Manual ATC audio to a USB or Bluetooth speaker via `mpv` (portal / Settings → ATC). Pair Bluetooth on the web portal; on-screen ATC offers quick reconnect. Channel lists are fetched from LiveATC search via `curl-cffi` (Cloudflare bypass; approach inspired by [pyliveatc](https://github.com/LeMetadatarr/pyliveatc)). Install: `sudo apt install mpv bluez libspa-0.2-bluetooth pulseaudio-utils`. `install-pi.sh` disables the Pi desktop Bluetooth panel plugin so its “Connection successful” dialog does not steal fullscreen focus. |
...
| 1   | **Enclosure / Stand** (no speaker)            | 3D-printed desktop enclosure **without** USB-speaker bay.                                                                                                                         | [MakerWorld — no speaker](https://makerworld.com/en/models/3024952-flightscnrpi-large-ads-b-traffic-sweeping-radar#profileId-3399104) |
| 0–1 | **Enclosure / Stand** (with speaker)          | Alternate print profile **with** USB-speaker bay — use instead of the no-speaker enclosure when installing a USB speaker.                                                         | [MakerWorld — with speaker](https://makerworld.com/en/models/3024952-flightscnrpi-large-ads-b-traffic-sweeping-radar#profileId-3532792) |
...
| 0–1 | **USB speaker** (optional)                    | For built-in ATC / hourly chime with the speaker enclosure. Tested: HONKYOB USB Mini Speaker.                                                                                     | [Amazon](https://www.amazon.com/HONKYOB-Speaker-Computer-Multimedia-Notebook/dp/B075M7FHM1)                                   |
