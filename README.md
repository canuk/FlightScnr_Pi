...
| **[LiveATC](https://www.liveatc.net/)**                                | Optional                 | Manual ATC audio to a USB or Bluetooth speaker via `mpv` (portal / Settings → ATC). Pair Bluetooth on the web portal; on-screen ATC offers quick reconnect. Install: `sudo apt install mpv bluez libspa-0.2-bluetooth pulseaudio-utils`. `install-pi.sh` disables the Pi desktop Bluetooth panel plugin so its “Connection successful” dialog does not steal fullscreen focus. |
